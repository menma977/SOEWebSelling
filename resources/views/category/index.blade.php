      ...
      <table id="user-table" class="table display responsive w-100" style="width:100%;">
        ...
        <template id="template-user-row">
          <tr>
            ...
            <td>
              <a class="btn btn-danger btn-sm btn-block delete" href="{{ route('category.destroy', '##id##') }}">
                Delete
              </a>
            </td>
          </tr>
  ...
  <script>
    ...
    $(function () {
      ...
      $(document).on("click", ".edit", function () {
        const url = "{{ route('category.store', '#id') }}".replace('#id', $(this).attr('id'));
        $('#_up').attr('action', url);
    ...
    async function refreshTable(e) {
      ...
      const response = await fetch("{{ route('category.show', '##filter##') }}".replace("##filter##", filter), {
        method: 'GET',
        headers: {
